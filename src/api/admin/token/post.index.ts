...
import { TokenCreateValidation } from "./token.validation";
...
export async function handleRequest(req: Request): Promise<Response> {
    try {
        ...
        parse(TokenCreateValidation, { active: active, name: name, public_token: publicToken, query_token: queryToken, description: description });

        const query = "INSERT INTO token (active, name, public_token, query_token, description) VALUES (:active, :name, :public_token, :query_token, :description);";
        const params = {
            ":active": active,
            ":name": name,
